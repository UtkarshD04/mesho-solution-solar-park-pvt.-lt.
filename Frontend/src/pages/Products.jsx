      ...
      <section className="max-w-7xl mx-auto px-6 py-14">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-10">
          ...
          <p className="text-sm text-gray-400 font-medium">
            {visible.length === products.length ? `All ${products.length} products` : `${visible.length} found`}
          </p>
        </div>
        ...
        {visible.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-16 text-center">
            ...
            <button
              className="rounded-xl px-6 py-3 text-sm font-black uppercase tracking-wider text-white transition hover:opacity-90"
...
function ProductCard({ product, onClick }) {
  return (
    <div
      onClick={onClick}
      className="group bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative"
    >
      {/* Coming Soon Ribbon */}

...
export default function Technology() {
  ...
  return (
    <div className="bg-white min-h-screen text-slate-900 overflow-x-hidden">
      ...
      {/* ── ADDITIONAL INNOVATIONS GRID ── */}
      <section ref={innovRef} className="py-24 bg-slate-50 border-t border-slate-100 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-16 relative">
          <div className={`text-center mb-16 transition-all duration-1000 ${innovInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            <span className="text-[#20b2aa] text-xs font-bold uppercase tracking-[0.3em]">Beyond the Hardware</span>
            <h2 className="mt-3 text-4xl lg:text-5xl font-black text-slate-900 tracking-tight">More MYZO Innovations</h2>
            <p className="mt-4 text-slate-500 text-sm max-w-xl mx-auto leading-relaxed">
              Our technology advantage goes deeper than the four core pillars — here are additional capabilities built into every MYZO system.
            </p>
            <div className="w-14 h-1 bg-gradient-to-r from-[#20b2aa] to-[#033e74] rounded-full mx-auto mt-5" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {innovations.map((inn, i) => (
              <div
                key={i}
                className={`group bg-white border border-slate-200 hover:border-[#20b2aa] rounded-2xl p-7 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1 ${innovInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
                style={{ transitionDelay: `${i * 80 + 200}ms` }}
              >
                <div className="text-3xl mb-4">{inn.icon}</div>
                <h3 className="text-lg font-extrabold text-[#033e74] mb-2 group-hover:text-[#20b2aa] transition-colors duration-300">{inn.title}</h3>
                <p className="text-slate-500 text-xs leading-relaxed">{inn.desc}</p>
      ...
      <section ref={ctaRef} className="py-28 relative overflow-hidden">
        ...
        <div className={`max-w-4xl mx-auto px-6 text-center relative transition-all duration-1000 ${ctaInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
          ...
          <h2 className="text-4xl lg:text-5xl font-black text-white leading-tight mb-5">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#20b2aa] to-teal-300">While You Sleep</span>
          </h2>
          <p className="text-white/60 text-base max-w-xl mx-auto leading-relaxed mb-10">
            From 5kWh residential stacks to 500kWh commercial BESS — MYZO technology scales seamlessly with your energy needs, backed by real engineering and real data.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <a href="/products" className="inline-flex items-center gap-2 bg-[#20b2aa] hover:bg-[#1a938c] text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-teal-500/30 text-sm">
              Explore MYZO Products
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
            </a>
